...
- (NSString *)destroyRoom:(XMPPJID *)jid
{
    NSString *iqID = [xmppStream generateUUID];
    ...
    dispatch_block_t block = ^{ @autoreleasepool {
        ...
        NSXMLElement *item = [NSXMLElement elementWithName:@"destroy"];
...
- (void)xmppStream:(XMPPStream *)sender didReceiveMessage:(XMPPMessage *)message
{
    ...
    NSString *exten = [x xmlns];
        ...
        // 210 - Inform user that service has assigned or modified occupant's roomnick.
        // 303 - Inform all occupants of new room nickname.
    if([exten isEqualToString:XMPPMUCUserNamespace])
    {
        NSString *affiliation = [[x elementForName:@"item"]attributeStringValueForName:@"affiliation"];
