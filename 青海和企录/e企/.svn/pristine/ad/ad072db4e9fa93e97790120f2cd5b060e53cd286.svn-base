...
//    self.textColor = [UIColor whiteColor];
    ...
    self.inactiveLinkAttributes = [NSDictionary dictionaryWithDictionary:mutableInactiveLinkAttributes];///<不活跃的
}
...
//                                                           range:NSMakeRange(0, [emojiText length])];
    ...
    for (NSTextCheckingResult *result in emojis) {
        ...
        NSString *faceIndex=[emojiKey substringWithRange:NSMakeRange(1, emojiKey.length-2)];
        if ([faceIndex intValue]) {
            //把之前[01]类似的表情替换汉字
            
            if ([faceIndex intValue]<=100) {
                emojiKey=[[ConstantObject sharedConstant].faceNameArray objectAtIndex:[faceIndex intValue]-1];
            }
...
- (void)setEmojiText:(NSString*)emojiText
{
    ...
    /**
     *  电话和邮箱都不需要所以maxIndex,设置为1
     */
    NSUInteger maxIndex = kURLActionCount;
//    maxIndex=1;
    for (NSUInteger i=0; i<maxIndex; i++) {
//        if (self.disableThreeCommon&&i<kURLActionCount-2) {
//            continue;
//        }
//        = {@"url->",@"email->",@"phoneNumber->",@"at->",@"poundSign->"};
        NSString *urlAction = kURLActions[i];
        if (i==1 && self.disableEmail) {
            continue;
        }
        if (i==2 && self.disablePhoneNumber) {
            continue;
        }
        if (i==3 && self.disableAt) {
            continue;
        }
        if (i==4 && self.disablePoundSign) {
            continue;
        }
...
//            NSString *actionString=[NSString stringWithFormat:@"%@这是网址",urlAction];
...
#pragma clang diagnostic ignored "-Wundeclared-selector"
    ...
    if (self.highlightedStrRange.length>1) {
        
        NSString *actionString=[self.text substringWithRange:self.highlightedStrRange];
        
        NSTextCheckingResult *aResult = [NSTextCheckingResult correctionCheckingResultWithRange:self.highlightedStrRange replacementString:actionString];
        [results addObject:aResult];
    }
