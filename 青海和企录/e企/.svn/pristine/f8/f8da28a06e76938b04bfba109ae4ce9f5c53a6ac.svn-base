...
#import "MWActivityIndicatorView.h"
...
static const CGFloat CSToastCornerRadius        = 3.0;
...
- (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration target:(id)target action:(SEL)action position:(id)position {
    UIView *toast = [self viewForMessage:message title:nil image:nil];
    [self showToast:toast duration:duration target:target action:action position:position];
}
...
- (void)showToast:(UIView *)toast duration:(NSTimeInterval)duration target:(id)target action:(SEL)action position:(id)point {
    toast.center = [self centerPointForPosition:point withToast:toast];
    toast.alpha = 0.0;
    ...
    [UIView animateWithDuration:CSToastFadeDuration
                     ...
                     animations:^{
                         ...
                         NSTimer *timer = [NSTimer scheduledTimerWithTimeInterval:duration target:target selector:action userInfo:toast repeats:NO];
                         // associate the timer with the toast view
                         objc_setAssociatedObject (toast, &CSToastTimerKey, timer, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
                     }];
...
- (void)makeToastActivityCircle:(NSString*)message {
    [self makeToastActivityCircle:CSToastActivityDefaultPosition message:message];
}

- (void)makeToastActivityCircle:(id)position message:(NSString *)message {
    // sanity
    UIView *existingActivityView = (UIView *)objc_getAssociatedObject(self, &CSToastActivityViewKey);
    if (existingActivityView != nil)
    {
        [self hideToast:existingActivityView];
    };
    
    CGRect activityFrame = CGRectMake(0, 0, CSToastActivityWidth, CSToastActivityHeight);
    ...
    if ([message length] != 0)
    {
        ...
        messageFrame = CGRectMake(0.0, 0.0, expectedSizeMessage.width + CSToastHorizontalPadding, expectedSizeMessage.height + CSToastVerticalPadding*2);
        activityFrame.size.width = messageFrame.size.width + messageFrame.size.height;
        activityFrame.size.height = messageFrame.size.height;
    }
    
    UIView *activityView = [[UIView alloc] initWithFrame:activityFrame];
    activityView.center = [self centerPointForPosition:position withToast:activityView];
    activityView.backgroundColor = [[UIColor darkGrayColor] colorWithAlphaComponent:CSToastOpacity];
    activityView.alpha = 0.0;
    activityView.autoresizingMask = (UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin | UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleBottomMargin);
    activityView.layer.cornerRadius = CSToastCornerRadius;
    ...
    CGRect frame = CGRectMake(0, 0, CSToastActivityWidth / 2, CSToastActivityWidth / 2);
    CGPoint center = CGPointMake(activityView.bounds.size.width / 2, activityView.bounds.size.height / 2);
    if (messageLabel)
    {
        frame = CGRectMake(0, 0, messageFrame.size.height / 1.5, messageFrame.size.height / 1.5);
        center = CGPointMake(messageFrame.size.height / 2, messageFrame.size.height / 2) ;
        messageLabel.center = CGPointMake(messageLabel.center.x + messageFrame.size.height, messageLabel.center.y+10);
        [activityView addSubview:messageLabel];
        
    }
    
    MWActivityIndicatorView *activityIndicatorView = [[MWActivityIndicatorView alloc] initWithFrame:frame];
