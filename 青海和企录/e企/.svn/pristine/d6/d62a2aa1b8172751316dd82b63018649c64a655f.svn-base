...
- (void)repositionImageView
{
    ...
    DDLogInfo(@"%f---%f----====%f====-----%f",borderWidth,borderWidth,self.frame.size.width,
          self.frame.size.height);
}
...
- (void)resetRatioControls
{
   CGRect actualImageRect = [self imageFrameFromImageViewWithAspectFitMode:self.imageView];
    DDLogInfo(@"----%f-----%f========%f--------%f----",actualImageRect.origin.x,actualImageRect.origin.y,actualImageRect.size.width,actualImageRect.size.width);
   // CGRect actualImageRect = CGRectMake(0, 0, self.frame.size.width,self.frame.size.height);
    ...
    DDLogInfo(@"----%f-----%f========%f--------%f----",frame.origin.x,frame.origin.y,frame.size.width,frame.size.width);
    
    [self.ratioControlsView setFrame:actualImageRect];
    DDLogInfo(@"----%f-----%f========%f--------%f----",actualImageRect.origin.x,actualImageRect.origin.y,actualImageRect.size.width,actualImageRect.size.width);
    [self.overlayView setFrame:ratioControlsView.bounds];
    
    DDLogInfo(@"self.ratioView.frame.size.width--------%f==========self.ratioView.frame.size.height%f",self.ratioControlsView.frame.size.width,self.ratioControlsView.frame.size.height);
...
- (CGRect)imageFrameFromImageViewWithAspectFitMode:(UIImageView *)theImageView
{
    ...
    DDLogInfo(@"%f-------%f-------",imageSize.width,imageSize.height);
    float viewRatio = self.frame.size.width / self.frame.size.height;
    
    DDLogInfo(@"imageRatio=== %f viewRatio===%f ",imageRatio,viewRatio);
    
    if (imageRatio < viewRatio)
    {
        
        float scale = self.frame.size.height / imageSize.height;
        DDLogInfo(@"-----------%f-------%f", self.frame.size.height , imageSize.height);
        float width = scale * imageSize.width;
        float topLeftX = .5 * (self.frame.size.width - width);
        DDLogInfo(@"%f*********44444*****%f********4444******%f",topLeftX,width,self.frame.size.height);
    ...
    else
    {
        ...
        DDLogInfo(@"%f**************%f**************%f",topLeftY,self.frame.size.width,height);
...
- (CGSize)sizeForRotatedImage:(UIImage *)imageToRotate
{
    ...
    DDLogInfo(@"outputSize....%f ------%f ====",outputSize.width,outputSize.height);
    return outputSize;
}
...
- (UIImage *)croppedImage:(UIImage *)imageToCrop
{
  ...
  //  CGRect actualCropRect = CGRectMake(0, 0, 320, 410);
    DDLogInfo(@"%f***********%f**************%f*************%f",actualCropRect.origin.x,actualCropRect.origin.y,actualCropRect.size.width,actualCropRect.size.height);
    UIImage *outputImage = nil;
...
- (void)panGesture:(UIPanGestureRecognizer *)recognizer
{
    ...
    if (ratioViewMovementType == AGMovementTypeHorizontally)
    {
        ...
        DDLogInfo(@"-------------%f-----------",self.ratioControlsView.frame.size.width);
